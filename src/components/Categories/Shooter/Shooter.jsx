...
export default function Shooter() {
    const [allGamesOnShooter, setallGamesOnShooter] = useState(null);
    let [itemNumOnShooter, setitemNumOnShooter] = useState(25);
   ...
   function checkingItemNumOfShooter(){
          if(itemNumOnShooter >= 78){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnShooter(itemNumOnShooter+=25);
              getAllGamesByPlatform(incNumberOfItemsOnShooter);
          }
      }
  
      function incNumberOfItemsOnShooter(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnShooter ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnShooter);
              console.log(arrOfGames);
              setallGamesOnShooter(arrOfGames);
      }
    ...
    return <>
      {allGamesOnShooter == null ? <LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnShooter.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfShooter} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
