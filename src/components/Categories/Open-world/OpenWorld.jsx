...
export default function OpenWorld() {
    const [allGamesOnOpenWorld, setallGamesOnOpenWorld] = useState(null);
    let [itemNumOnOpenWorld, setitemNumOnOpenWorld] = useState(25);
   ...
   function checkingItemNumOfOpenWorld(){
          if(itemNumOnOpenWorld >= 130){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnOpenWorld(itemNumOnOpenWorld+=25);
              getAllGamesByPlatform(incNumberOfItemsOnOpenWorld);
          }
      }
  
      function incNumberOfItemsOnOpenWorld(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnOpenWorld ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnOpenWorld);
              console.log(arrOfGames);
              setallGamesOnOpenWorld(arrOfGames);
      }
    ...
    return <>
      {allGamesOnOpenWorld == null ? <LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnOpenWorld.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfOpenWorld} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
