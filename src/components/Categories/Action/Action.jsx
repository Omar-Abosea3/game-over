...
export default function Action() {
    const [allGamesOnAction, setallGamesOnAction] = useState(null);
    let [itemNumOnAction, setitemNumOnAction] = useState(25);
   ...
   function checkingItemNumOfAction(){
          if(itemNumOnAction >= 259){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnAction(itemNumOnAction+=25);
              getAllGamesByPlatform(incNumberOfItemsOnAction);
          }
      }
  
      function incNumberOfItemsOnAction(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnAction ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnAction);
              console.log(arrOfGames);
              setallGamesOnAction(arrOfGames);
      }
    ...
    return <>
      {allGamesOnAction == null ? <LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnAction.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfAction} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
