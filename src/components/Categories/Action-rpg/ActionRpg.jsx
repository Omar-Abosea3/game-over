...
export default function ActionRpg() {
    const [allGamesOnActionRpg, setallGamesOnActionRpg] = useState(null);
    let [itemNumOnActionRpg, setitemNumOnActionRpg] = useState(22);
   ...
   function checkingItemNumOfActionRpg(){
          if(itemNumOnActionRpg >= 25){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnActionRpg(itemNumOnActionRpg+=25);
              getAllGamesByPlatform(incNumberOfItemsOnActionRpg);
          }
      }
  
      function incNumberOfItemsOnActionRpg(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnActionRpg ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnActionRpg);
              console.log(arrOfGames);
              setallGamesOnActionRpg(arrOfGames);
      }
    ...
    return <>
      {allGamesOnActionRpg == null ? <LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnActionRpg.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfActionRpg} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
