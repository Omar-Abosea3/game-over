...
export default function Pc() {
  ...
  const [allGamesOnBc, setallGamesOnBc] = useState(null);
  let [itemNumOnBc, setitemNumOnBc] = useState(25);
 ...
 function checkingItemNumOfBc(){
        if(itemNumOnBc >= 276){
            $('#addBtn').addClass('d-none');
        }else{
            setitemNumOnBc(itemNumOnBc+=25);
            getAllGamesByPlatform(incNumberOfItemsOnBc);
        }
    }

    function incNumberOfItemsOnBc(data){
        let arrOfGames = [];
            for( let i=0 ; i <= itemNumOnBc ; i++ ){
                arrOfGames.push(data[i]);
            }
            console.log(allGamesOnBc);
            console.log(arrOfGames);
            setallGamesOnBc(arrOfGames);
    }
  ...
  return <>
    {allGamesOnBc == null ? <LodingScrean/>: <div className="container py-5 px-3">
          <div className="row gy-3">
                {allGamesOnBc.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
              ...
              <button id='addBtn' onClick={checkingItemNumOfBc} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
          </div>
      </div>}
  </>
}
