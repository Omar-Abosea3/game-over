...
export default function RealaseDate() {
  
    const [allGamesOnReleaseDate, setallGamesOnReleaseDate] = useState(null);
    let [itemNumOnReleaseDate, setitemNumOnReleaseDate] = useState(25);
   ...
   function checkingItemNumOfReleaseDate(){
          if(itemNumOnReleaseDate >= 326){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnReleaseDate(itemNumOnReleaseDate+=25);
              getAllGamesByPlatform(incNumberOfItemsOnReleaseDate);
          }
      }
  
      function incNumberOfItemsOnReleaseDate(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnReleaseDate ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnReleaseDate);
              console.log(arrOfGames);
              setallGamesOnReleaseDate(arrOfGames);
      }
    ...
    return <>
      {allGamesOnReleaseDate == null ? <LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnReleaseDate.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfReleaseDate} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
