...
export default function Alphapetical() {
    const [allGamesOnAlphabetical, setallGamesOnAlphabetical] = useState(null);
    let [itemNumOnAlphabetical, setitemNumOnAlphabetical] = useState(25);
   ...
   function checkingItemNumOfAlphabetical(){
          if(itemNumOnAlphabetical >= 326){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnAlphabetical(itemNumOnAlphabetical+=25);
              getAllGamesByPlatform(incNumberOfItemsOnAlphabetical);
          }
      }
  
      function incNumberOfItemsOnAlphabetical(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnAlphabetical ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnAlphabetical);
              console.log(arrOfGames);
              setallGamesOnAlphabetical(arrOfGames);
      }
    ...
    return <>
      {allGamesOnAlphabetical == null ?<LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnAlphabetical.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfAlphabetical} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
