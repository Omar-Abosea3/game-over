...
export default function Relevence() {
    const [allGamesOnRelevence, setallGamesOnRelevence] = useState(null);
    let [itemNumOnRelevence, setitemNumOnRelevence] = useState(25);
   ...
   function checkingItemNumOfRelevence(){
          if(itemNumOnRelevence >= 326){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnRelevence(itemNumOnRelevence+=25);
              getAllGamesByPlatform(incNumberOfItemsOnRelevence);
          }
      }
  
      function incNumberOfItemsOnRelevence(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnRelevence ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnRelevence);
              console.log(arrOfGames);
              setallGamesOnRelevence(arrOfGames);
      }
    ...
    return <>
      {allGamesOnRelevence == null ? <LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnRelevence.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfRelevence} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
