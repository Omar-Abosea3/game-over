...
export default function Popularity() {
    const [allGamesOnPopularity, setallGamesOnPopularity] = useState(null);
    let [itemNumOnPopularity, setitemNumOnPopularity] = useState(25);
   ...
   function checkingItemNumOfPopularity(){
          if(itemNumOnPopularity >= 326){
              $('#addBtn').addClass('d-none');
          }else{
              setitemNumOnPopularity(itemNumOnPopularity+=25);
              getAllGamesByPlatform(incNumberOfItemsOnPopularity);
          }
      }
  
      function incNumberOfItemsOnPopularity(data){
          let arrOfGames = [];
              for( let i=0 ; i <= itemNumOnPopularity ; i++ ){
                  arrOfGames.push(data[i]);
              }
              console.log(allGamesOnPopularity);
              console.log(arrOfGames);
              setallGamesOnPopularity(arrOfGames);
      }
    ...
    return <>
      {allGamesOnPopularity == null ?<LodingScrean/>: <div className="container py-5 px-3">
            <div className="row gy-3">
                  {allGamesOnPopularity.map((game , index) =><div key={index} title={ "Available on "+ game.platform} className="col-lg-3 col-md-6 col-sm-12 col-12">
                ...
                <button id='addBtn' onClick={checkingItemNumOfPopularity} className='btn btn-outline-secondary'>More Games <i className='fa fa-arrow-right'></i></button>
            </div>
        </div>}
    </>
}
